...
import PhotoViewModal from "../../../../Modal/organization/photoViewModal";
import { ModalOpenCloseContext } from "@/contexts/modalContext/modalOpenCloseContext";
import AddaNewImageModal from "../../../../Modal/organization/addanewImageModal";
...
export interface PartnershipPhotosProps {
  profileType?: string;
  profileOrPartnershipId?: any;
  isCreator: any;
}

const PartnershipPhotos: React.FC<PartnershipPhotosProps> = (props) => {
  ...
  const renderPhotos = (data: any) => {
    return (
      <div
        ...
        className="show-uploaded-image-container relative mb-5 ml-2 mr-2 mt-2 block w-3/12 max-w-[18rem] transform rounded-lg bg-white shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)] transition duration-500 hover:scale-105 hover:cursor-pointer dark:bg-neutral-700"
        onKeyDown={(e: any) => {
          if (e.key === 'Enter') {
            e.preventDefault();
      ...
      >
        ...
        <div className="absolute bottom-0 right-0 my-auto mb-2 mr-2 flex flex-row items-center align-middle">
...
export default PartnershipPhotos;
